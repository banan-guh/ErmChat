...
void main() {
  ...
  group('createDeleteSubscription', () {
    test('sends POST with channel.chat.message_delete body', () async {
      late http.Request captured;
      final api = createApi(
        (req) => captured = req,
        respond: () => http.Response('Accepted', 202),
      ...
      expectAuthHeaders(captured);
      final body = jsonDecode(captured.body) as Map<String, dynamic>;
      expect(body['type'], 'channel.chat.message_delete');
      expect(body['condition'], {'broadcaster_user_id': 'b1', 'user_id': 'u1'});
      expect(body['transport'], {'method': 'websocket', 'session_id': 's1'});
    });
  ...
  group('createBanSubscription', () {
    test('sends POST with channel.ban body and moderator_user_id', () async {
      late http.Request captured;
      final api = createApi(
        (req) => captured = req,
        respond: () => http.Response('Accepted', 202),
      ...
      expect(
        await api.createBanSubscription(
          auth: auth,
          sessionId: 's1',
          broadcasterUserId: 'b1',
          moderatorUserId: 'm1',
