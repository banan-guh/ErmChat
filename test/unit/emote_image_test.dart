...
void main() {
  ...
  group('sniffEmoteFormat', () {
    ...
    test('detects AVIF brands', () {
      final avif = Uint8List.fromList([
        0, 0, 0, 32, 0x66, 0x74, 0x79, 0x70, // ftyp
        0x61, 0x76, 0x69, 0x66, // avif
      ]);
      final avis = Uint8List.fromList([
        0, 0, 0, 32, 0x66, 0x74, 0x79, 0x70, // ftyp
        0x61, 0x76, 0x69, 0x73, // avis
      ]);
      expect(sniffEmoteFormat(avif), EmoteFormat.avif);
      expect(sniffEmoteFormat(avis), EmoteFormat.avif);
    });
    ...
    test('everything else is other', () {
      ...
      final riffWebp = Uint8List.fromList('RIFFxxxxWEBP'.codeUnits);
      expect(sniffEmoteFormat(png), EmoteFormat.other);
      expect(sniffEmoteFormat(riffWebp), EmoteFormat.other);
  ...
  group('EmoteClipRegistry', () {
    ...
    test('re-acquires (and re-fetches) after the last release', () async {
      var fetches = 0;
      EmoteClipRegistry.debugFetchOverride = (url) async {
        fetches++;
        return Uint8List.fromList('GIF89a'.codeUnits);
