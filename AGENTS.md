...
- `lib/services/twitch_oauth.dart` - OAuth implicit grant flow (browser-based login, fragment parsing); `startFlow({bool ephemeral})` - ephemeral (cookie-less) sessions are used for the switch-account re-auth path so Twitch skips the "hi again - not you? log out" interstitial (whose app-link button can hand the flow to the installed Twitch app). On Android the flow runs through `MainActivity` instead of `flutter_web_auth_2`: the native side launches the auth URL in a session-bound Custom Tab (`CustomTabsIntent.Builder(session)` keeps every navigation inside the tab, so Android App Links can't hand off to the Twitch app) and forwards the `ermchat://` redirect back via the `ermchat/oauth` MethodChannel (`launchCustomTab` / `onRedirect`); iOS keeps `flutter_web_auth_2` (ASWebAuthenticationSession)
