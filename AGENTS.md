...
- `lib/services/emote_manager.dart` - `ChangeNotifier`-based emote caching. The metadata TTL (12h wifi / 24h cellular on high, flat 24h on medium, infinite on low/nothing; connectivity_plus probe, cached 60s) is a *refresh* flag only - it re-runs the provider rake for stale channels, never wipes image files. TTL-gated fetches go through a serialized queue with a 1.5s stagger (the one-by-one "rake"); fresh caches skip the network entirely, Twitch channel emotes refresh in the background per open. `updateSevenTvEmotes` applies live WebSocket deltas; `_reconcileSevenTv` fetches the 7TV set once per launch on a fresh cache (medium/high only) and diffs it against the loaded cache to apply add/remove/rename deltas at startup. `markEmoteViewed` feeds the usage registry on render. `cacheCap` forwards the settings cap to `EmoteCacheManager`; `startCacheGc` runs the one-time migrations and registers the usage registry as the cache's priority source (no periodic sweep)
- `lib/services/emote_cache_manager.dart` - dedicated emote image disk cache singleton used by every emote render (`EmoteImage` fetches bytes through it; badges/avatars via `CachedNetworkImageProvider.defaultCacheManager`); never exceeds the settings cap (`maxObjects`): a write is only accepted while the repo count (plus in-flight writes) is below the cap, and a full cache serves new emotes from an OS temp file (FIFO of 8) instead of persisting, so bursts can't overshoot. `isFull()` backs the precacher's skip decision. `enforceNow()` (settings Apply / startup) evicts down to a newly reduced cap by priority (`lastUsedAt` registry lookup, falling back to the file's touched time); cap 0 evicts everything. The 2000-file/30d flutter_cache_manager config is a safety net only. `stats()` returns `EmoteCacheStats` (file count + total bytes) for the settings footer; a `forTesting` ctor accepts a custom `Config`
...
- `lib/services/emote_providers/seven_tv_emotes.dart` - 7TV global + channel emotes (AVIF preferred over WEBP: 7TV serves every emote as AVIF and it is smaller; `isAnimated` is derived from the presence of a GIF variant)
...
- `lib/widgets/emote_text.dart` - emote-aware text rendering with inline image spans, clickable links, zero-width overlay support (renders via `EmoteImage`)
- `lib/widgets/emote_image.dart` - the emote renderer: fetches bytes through `EmoteCacheManager` (cap + overflow paths apply), sniffs the format (`GIF8` / `ftypavif` / other), and decodes animated GIFs in pure Dart (`package:image` in an isolate; correct disposal/transparency, file-accurate per-frame delays) and AVIF via `flutter_avif`/libavif (static + animated). Playback is driven by a per-widget `Ticker`; decoded frames are shared per URL via a refcounted `EmoteClipRegistry` (disposed when the last widget releases) with injectable fetch/decode overrides for tests. Static images go through the engine codec (safe for single frames). This bypasses the engine's animated-image codec entirely - it mis-renders fully-transparent frames and misreads some frame delays (DankChat-compatible timing requires the pure-Dart path). No other emote render site uses `CachedNetworkImage`/`Image.network` anymore
...
- `gif_fixture.dart` - minimal GIF89a byte builder (transparent-frame variants, per-frame delays) shared by the codec repro and emote-image tests
...
- `seven_tv_emotes_test.dart` - 15 tests: 7TV emote provider + resolution variants (no 4x), AVIF preference over WEBP with fallback, GIF-variant animated detection
- `emote_image_test.dart` - 13 tests: format sniffing, clip registry (dedup, release/re-fetch, error retry), production GIF/PNG decode paths, widget placeholder/error/frame-advance/shared-fetch behavior
- `animated_codec_repro_test.dart` - 2 tests: documents that the engine codec does NOT throw on fully-transparent GIF frames and composites transparency over the previous frame (the nuke the EmoteImage migration works around)
...
- Android build quirks (all handled in `android/build.gradle.kts`, do not "fix" them away): (1) every subproject's Kotlin `jvmTarget` is aligned to its Java `compileOptions` target because plugin modules pin Java 8/11/17 while Kotlin defaults to the running JDK (JVM-target consistency failure); (2) `flutter_avif_android` (2022-era plugin) is compiled with `-language-version 1.9` because it trips the strict interface-member redeclaration check of the current language version, and its `compileSdk` is raised from 31 to 34 because its transitive androidx deps require 34+; (3) AGP 9 legacy mode (`android.newDsl=false`, the template default) copies Kotlin classes into the javac output dir, so each `compile*JavaWithJavac` task deletes output files that are exact copies of the `compile*Kotlin` output (otherwise `bundleLibCompileToJar` fails with "already contains entry")
